...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, DSConv, Focus,
                 ...
                 C2f_KAT, C2f_Faster_KAN, C2f_DCMB, C2f_DCMB_KAN, C2f_GlobalFilter, CASFN, RepHMS, C2f_SAVSS, C2f_MambaOut,
                 ...
                 C2f_LWGA, C2f_CSSC, C2f_CNCM, C2f_HFRB, C2f_EVA, C2f_RMBC, C2f_RMBC_LA, C2f_SFMB, ESMoE, FSConv, C2f_MFEB, C2f_PartialNetBlock, C2f_DRG, C2f_GLGM,
                 C2f_MAC, C2f_TransMixer, C2f_DEGConv, C2f_GLSS2D, C2f_SPJFB, C2f_SpatialOnly, C2f_FreqOnly, C2f_StaticFreqFilter, C2f_DynamicFilterNoBasis):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, DWRC3, C3_DWR, C2f_DWR, C3_DCNv2_Dynamic, C2f_DCNv2_Dynamic,
                     ...
                     C2f_Strip, C2f_StripCGLU, C2f_KAT, C2f_Faster_KAN, C2f_DCMB, C2f_DCMB_KAN, C2f_GlobalFilter, CASFN, C2f_SAVSS, C2f_MambaOut,
                     C2f_EfficientVIM, C2f_EfficientVIM_CGLU, CSP_MSCB_SC, C2f_MambaOut_UniRepLK, C2f_IEL, IELC3, C2f_RCB, C2f_FAT, C2f_LEGM, C2f_MobileMamba,
                     ...
                     C2f_RMBC, C2f_RMBC_LA, C2f_SFMB, C2f_MFEB, C2f_PartialNetBlock, C2f_DRG, C2f_GLGM, C2f_MAC,  C2f_TransMixer, C2f_DEGConv, C2f_GLSS2D, C2f_SPJFB, C2f_SpatialOnly, C2f_FreqOnly, C2f_StaticFreqFilter, C2f_DynamicFilterNoBasis):
                ...
                n = 1
                   ...
                   TransformerEncoderLayer_LRSA, TransformerEncoderLayer_MALA, TransformerEncoderLayer_EGSA, TransformerEncoderLayer_SWSA, GIFI,
        ...
        # --------------ASF--------------
            ...
            c2 = make_divisible(min(args[0], max_channels) * width, 8)
            args = [c1, c2]
        elif m in {MFPM, ERM, CAFM_Fusion}:
            c1 = [ch[x] for x in f]
            c2 = make_divisible(min(args[0], max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
        else:
        ...
        if isinstance(c2, list) and m not in {ChannelTransformer, PyramidContextExtraction, CrossLayerChannelAttention, CrossLayerSpatialAttention, MutilScaleEdgeInfoGenetator}:
            ...
            if hasattr(m, '__name__') and 'FreqOnly' in m.__name__:
                print(f"\n[DEBUG] C2f_FreqOnly debug:")
                print(f"  m={m}")
                print(f"  n={n}")
                print(f"  args={args}")

...
__all__ = ['Ghost_HGBlock', 'Rep_HGBlock', 'DWRC3', 'C3_DWR', 'C2f_DWR', 'BasicBlock_Ortho', 'BottleNeck_Ortho',
           ...
           'C2f_Strip', 'C2f_StripCGLU', 'MultiScalePCA', 'FSA', 'MultiScalePCA_Down', 'C2f_KAT', 'C2f_Faster_KAN', 'C2f_DCMB', 'C2f_DCMB_KAN', 'C2f_GlobalFilter', 'CASFN', 'HAFB',
           ...
           'FSConv', 'MFPM', 'ERM', 'C2f_MFEB', 'C2f_PartialNetBlock', 'C2f_DRG', 'C2f_GLGM', 'C2f_MAC', 'C2f_TransMixer', 'C2f_DEGConv', 'C2f_GLSS2D', 'C2f_SPJFB', 'C2f_SpatialOnly', 'C2f_FreqOnly', 'C2f_StaticFreqFilter', 'C2f_DynamicFilterNoBasis'
           ]
    ...
    def __init__(self, in_channels, kernel_size=3, stride=1):
        ...
        self.up_dwc = nn.Sequential(
            ...
            nn.Conv2d(
                ...
                groups=in_channels, stride=stride, padding=1
            ),
...
class TokenMixer_For_Gloal_Spatial(nn.Module):
    """纯空间域全局token混合 - 用大感受野的空洞卷积替代FFT"""

    def __init__(self, dim):
        super().__init__()
        self.dim = dim
        self.CDilated_3 = nn.Conv2d(self.dim, self.dim, 3, stride=1, padding=3, dilation=3, groups=self.dim)
        self.CDilated_5 = nn.Conv2d(self.dim, self.dim, 3, stride=1, padding=5, dilation=5, groups=self.dim)

    def forward(self, x):
        x1, x2 = x.chunk(2, dim=1)
        cd3 = self.CDilated_3(x1)
        cd5 = self.CDilated_5(x2)
        x = torch.cat([cd3, cd5], dim=1)
        return x


class SpatialOnly_Mixer(nn.Module):
    """纯空间域Mixer：两个分支都使用空间卷积"""

    def __init__(self, dim):
        super().__init__()
        self.dim = dim
        self.mixer_local = TokenMixer_For_Local(dim=self.dim)
        self.mixer_global = TokenMixer_For_Gloal_Spatial(dim=self.dim)
        self.ca_conv = nn.Sequential(nn.Conv2d(2 * dim, dim, 1))
        self.ca = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
...
class SpatialOnly_Block(nn.Module):
    def __init__(self, dim, norm_layer=nn.BatchNorm2d):
        super().__init__()
        self.dim = dim
        self.norm1 = norm_layer(dim)
        self.norm2 = norm_layer(dim)
        self.mixer = SpatialOnly_Mixer(dim=self.dim)
        self.ffn = SFHF_FFN(dim=self.dim)
        self.beta = nn.Parameter(torch.zeros((1, dim, 1, 1)), requires_grad=True)
        self.gamma = nn.Parameter(torch.zeros((1, dim, 1, 1)), requires_grad=True)
...
class TokenMixer_For_Local_Freq(nn.Module):
    """纯频域局部token混合 - 用高频分量提取替代空间卷积"""
    ...
    def forward(self, x):
        ...
        ffted = torch.fft.rfft2(x, norm='ortho')
        # 高通滤波：保留高频，抑制低频
        h_freq, w_freq = ffted.shape[-2], ffted.shape[-1]
        mask = torch.ones_like(ffted)
        mask[:, :, h_freq // 4:3 * h_freq // 4, w_freq // 4:3 * w_freq // 4] = 0.1
        ffted = ffted * mask
        output = torch.fft.irfft2(ffted, s=(h, w), norm='ortho')
        return output


class FreqOnly_Mixer(nn.Module):
    """纯频域Mixer：两个分支都使用频域处理"""

    def __init__(self, dim):
        super().__init__()
        self.dim = dim
        self.mixer_local_freq = TokenMixer_For_Local_Freq(dim=self.dim)
        self.mixer_global_freq = TokenMixer_For_Gloal(dim=self.dim)
        self.ca_conv = nn.Sequential(nn.Conv2d(2 * dim, dim, 1))
        self.ca = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(2 * dim, dim, kernel_size=1),
    ...
    def forward(self, x):
        x = self.conv_init(x)
        x1, x2 = torch.split(x, self.dim, dim=1)
        x_local = self.mixer_local_freq(x1)
        x_global = self.mixer_global_freq(x2)
        x = torch.cat([x_local, x_global], dim=1)
        x = self.gelu(x)
        x = self.ca(x) * x
        x = self.ca_conv(x)
...
class FreqOnly_Block(nn.Module):
    def __init__(self, dim, norm_layer=nn.BatchNorm2d):
        super().__init__()
        self.dim = dim
        self.norm1 = norm_layer(dim)
        self.norm2 = norm_layer(dim)
        self.mixer = FreqOnly_Mixer(dim=self.dim)
        self.ffn = SFHF_FFN(dim=self.dim)
        self.beta = nn.Parameter(torch.zeros((1, dim, 1, 1)), requires_grad=True)
        self.gamma = nn.Parameter(torch.zeros((1, dim, 1, 1)), requires_grad=True)
...
# 你需要从 ultralytics 导入 C2f，或者确保 C2f 已经在当前命名空间
# from ultralytics.nn.modules import C2f  # 根据你的实际路径导入

class C2f_SFHF(C2f):
    """原始 SFHF 版本"""

    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        # 先调用父类初始化，确保 self.c 被定义
        super().__init__(c1, c2, n, shortcut, g, e)
        # 然后用 SFHF_Block 替换 m
        self.m = nn.ModuleList(SFHF_Block(self.c) for _ in range(n))


class C2f_SpatialOnly(C2f):
    """纯空间域版本"""

    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(SpatialOnly_Block(self.c) for _ in range(n))


class C2f_FreqOnly(C2f):
    """纯频域版本"""

    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(FreqOnly_Block(self.c) for _ in range(n))
...
class DynamicFilter(nn.Module):
    def __init__(self, dim, size=14, expansion_ratio=2, reweight_expansion_ratio=.25,
                 ...
                 bias=False, num_filters=4, weight_resize=False, drop=0.,
                 **kwargs):
        super().__init__()
        size = to_2tuple(size)
        self.size = size[0]
...
class StaticFreqFilter(nn.Module):
    def __init__(self, dim, size=14, expansion_ratio=2,
                 act1_layer=StarReLU, act2_layer=nn.Identity,
                 bias=False, weight_resize=False, drop=0., **kwargs):
        super().__init__()
        size = to_2tuple(size)
        self.size = size[0]
        self.filter_size = size[1] // 2 + 1
        ...
        self.act1 = act1_layer()
        self.complex_weights = nn.Parameter(
            torch.randn(self.size, self.filter_size, 1, 2, dtype=torch.float32) * 0.02)
        self.act2 = act2_layer()
        self.pwconv2 = nn.Linear(self.med_channels, dim, bias=bias)

    def forward(self, x):
        B, H, W, _ = x.shape
        x = self.pwconv1(x); x = self.act1(x)
        x = torch.fft.rfft2(x, dim=(1, 2), norm='ortho')
        if self.weight_resize:
            cw = resize_complex_weight(self.complex_weights, x.shape[1], x.shape[2])
            cw = torch.view_as_complex(cw.contiguous())
        else:
            cw = torch.view_as_complex(self.complex_weights)
        weight = cw.view(1, x.shape[1], x.shape[2], 1).expand(-1, -1, -1, self.med_channels)
        x = x * weight
        x = torch.fft.irfft2(x, s=(H, W), dim=(1, 2), norm='ortho')
        x = self.act2(x); x = self.pwconv2(x)
        return x

class DynamicFilterNoBasis(nn.Module):
    """
    消融：动态滤波但无基分解（N=1）
    有动态门控，但只有一个滤波器，不用基分解
    """
    def __init__(self, dim, size=14, expansion_ratio=2, reweight_expansion_ratio=.25,
                 act1_layer=StarReLU, act2_layer=nn.Identity,
                 bias=False, weight_resize=False, drop=0., **kwargs):
        ...
        # N=1：MLP输出med_channels（不是num_filters * med_channels）
        self.reweight = DynamicFilterMlp(dim, reweight_expansion_ratio, self.med_channels)

        # N=1：只有一个滤波器
        self.complex_weights = nn.Parameter(
            torch.randn(self.size, self.filter_size, 1, 2,
                       dtype=torch.float32) * 0.02)
    ...
    def forward(self, x):
        ...
        # 动态门控：输出[B, 1, med_channels]
        routeing = self.reweight(x.mean(dim=(1, 2))).view(B, 1, -1).softmax(dim=1)
        ...
        routeing = routeing.to(torch.complex64)
        # einsum: [B,1,med] @ [H,W,1] → [B,H,W,med]
        weight = torch.einsum('bfc,hwf->bhwc', routeing, complex_weights)
...
class C2f_DynamicFilterNoBasis(C2f):
    def __init__(self, c1, c2, n=1, size=None, shortcut=False, g=1, e=0.5,
                 weight_resize=False, drop=0., **kwargs):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(MetaFormerBlock(
                dim=self.c,
                token_mixer=partial(DynamicFilterNoBasis, size=size, weight_resize=weight_resize, drop=drop),
            ) for _ in range(n))

class C2f_StaticFreqFilter(C2f):
    def __init__(self, c1, c2, n=1, size=None, shortcut=False, g=1, e=0.5,
                 weight_resize=False, drop=0., **kwargs):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(MetaFormerBlock(
                dim=self.c,
                token_mixer=partial(StaticFreqFilter, size=size, weight_resize=weight_resize, drop=drop),
            ) for _ in range(n))

class CASFN(C2f):
    def __init__(self, c1, c2, n=1, size=None, shortcut=False, g=1, e=0.5,
                 num_filters=4, weight_resize=False):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(MetaFormerBlock(
                dim=self.c,
                token_mixer=partial(DynamicFilter, size=size, num_filters=num_filters, weight_resize=weight_resize),
            ) for _ in range(n))
######################################## Global Filter Networks for Image Classification end ########################################
...
class MFPM(nn.Module):
    ...
    def forward(self, inputs):
        ...
        # 统一尺寸：将 g 上采样到 x 的尺寸（或反过来）
        if g.shape[2:] != x.shape[2:]:
            g = F.interpolate(g, x.shape[2:], mode='bilinear', align_corners=False)
            # 或者：x = F.interpolate(x, g.shape[2:], mode='bilinear', align_corners=False)

        fusion = g + x  # 现在尺寸相同，可以相加
...
class ERM(nn.Module):
    ...
    def forward(self, inputs):
        ...
        fusion = self.conv_1(g + x)  # 现在尺寸相同
        fusion = self.ea(fusion)

        # 后续代码依赖 x 的 (h, w)，所以 g 必须对齐到 x

...
class EDFFN(nn.Module):
    def __init__(self, dim, ffn_expansion_factor=2, bias=False, low_rank_ratio=0.25):
        """
        dim: 输入特征通道数
        ffn_expansion_factor: FFN隐藏通道扩展倍数
        low_rank_ratio: 低秩比例，0 < r <= 1
        """
        ...
        # 频域门控参数: [1, dim, 1, 1, patch_size, patch_size//2+1]
        # 增加 batch 和 spatial 维度为1，便于广播到6维张量
        self.fft = nn.Parameter(torch.ones((1, dim, 1, 1, self.patch_size, self.patch_size // 2 + 1)))
    ...
    def forward(self, x):
        ...
        # 分块: [b, c, h_blocks, w_blocks, ph, pw]
        x_patch = rearrange(
            x, 'b c (h ph) (w pw) -> b c h w ph pw',
            ph=self.patch_size, pw=self.patch_size
        )

        # 频域: [b, c, h_blocks, w_blocks, ph, pw//2+1]  ← 6维
        x_patch_fft = torch.fft.rfft2(x_patch.float())

        # 低秩裁剪 (保持6维)
        rank = max(1, int(self.dim * self.low_rank_ratio))
        x_patch_fft_lowrank = x_patch_fft[:, :rank, ...] * self.fft[:, :rank, ...]

        # 填充剩余通道为0 (保持6维)
        if rank < self.dim:
            zeros = torch.zeros(
                (b, self.dim - rank, x_patch_fft.shape[2], x_patch_fft.shape[3],
                 x_patch_fft.shape[4], x_patch_fft.shape[5]),
                dtype=x_patch_fft.dtype, device=x_patch_fft.device
            )
            x_patch_fft = torch.cat([x_patch_fft_lowrank, zeros], dim=1)
        else:
            x_patch_fft = x_patch_fft_lowrank
